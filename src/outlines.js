...
const getContent = async specIndex => {
    let specs = [];
    let referenceSuccincts = {};
    for (const specUrl of specIndex) {
        const spec = fse.readJsonSync(path.resolve(process.argv[2], specUrl));
        specs.push(specUrl);
        console.log("\n**", spec.title, "**");
        await getKeywordResources(spec.keywordResources, referenceSuccincts);
        await getBcvResources(spec.bcvResources, referenceSuccincts);
        await getLemmaResources(spec.lemmaResources, referenceSuccincts);
        await getBibles(spec.bibles, referenceSuccincts);
        const outline = {
            title: spec.title,
            lang: spec.baseLanguageCode,
